 ...
 * @throws {Error} Se API não responder ou estrutura for diferente
 */
const getCurrencyValues = async (coin) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`API retornou status ${response.status} para ${coin.code}`);
    }
    ...
    // Retorna valor padrão para evitar quebra da Promise.all
    return {
      coin: coin.name,
      code: coin.code,
      value: 0,
...
const loadCoinValues = async () => {
  ...
  return values
    .filter((item) => item.status === "fulfilled" && item.value && !item.value.error)
    .map((item) => item.value)
...
const preencherMoedasFiat = () => {
  const selectFiat = document.getElementById("select2");

  if (!selectFiat) {
    console.error("Elemento select2 não encontrado");
    return;
  }

  selectFiat.innerHTML = "";

  const moedasFiat = [
    { label: "Real (BRL)", value: 1 },
    { label: "Dólar (USD)", value: 5.2 },
    { label: "Euro (EUR)", value: 5.7 },
  ];

  moedasFiat.forEach((moeda) => {
    selectFiat.appendChild(new Option(moeda.label, moeda.value));
  });
};

const getCoinByCode = (code) => currentCryptoCoins.find((coin) => coin.code === code);

/**
 * Inverte a direção de conversão (cripto→fiat ou fiat→cripto)
 */
const inverter = () => {
  ...
  // Atualizar título
  titulo.textContent = estadoConversor === "cripto-para-fiat" 
    ? "Converta criptomoeda em moeda fiduciária"
    : "Converta moeda fiduciária em criptomoeda";
  
  limparResultado();
  result.textContent = "SEU VALOR É...";
};

/**
 * Realiza a conversão entre criptomoedas e moedas fiduciárias
 */
const converter = async () => {
  const result = document.getElementById("resultado");
  const cryptoSelect = document.getElementById("select1");
  const fiatSelect = document.getElementById("select2");
  const amountInput = document.getElementById("numberCripto");
  
  if (!result || !cryptoSelect || !fiatSelect || !amountInput) {
    console.error("Elementos necessários não encontrados");
    return;
  }
  
  const quantidade = parseFloat(amountInput.value);
  const codigoCripto = cryptoSelect.value;
  const taxaFiat = parseFloat(fiatSelect.value);

  if (!codigoCripto) {
    result.textContent = "Selecione uma criptomoeda";
    return;
  }

  const moedaSelecionada = getCoinByCode(codigoCripto);

  if (!moedaSelecionada) {
    result.textContent = "Criptomoeda inválida";
    return;
  }
  ...
  try {
    const cotacao = await getCurrencyValues(moedaSelecionada);
    precoCripto = cotacao.value;
  } catch (error) {
    result.textContent = "Não foi possível obter a cotação da criptomoeda";
    return;
  }
  
  if (isNaN(quantidade) || quantidade <= 0 || isNaN(precoCripto) || isNaN(taxaFiat) || taxaFiat <= 0) {
    result.textContent = "Valores inválidos";
    return;
  }
  
  // Realizar cálculo de conversão
  let resultadoConversao;
  if (estadoConversor === "cripto-para-fiat") {
    resultadoConversao = ((quantidade * precoCripto) / taxaFiat).toFixed(2);
  } else {
    resultadoConversao = ((quantidade * taxaFiat) / precoCripto).toFixed(8);
  }
  
  const moedaDestino = fiatSelect.options[fiatSelect.selectedIndex]?.text || "moeda escolhida";
  result.textContent = `${resultadoConversao} ${moedaDestino}`;
};

/**
 * Gera as opções de seleção de moedas no dropdown
 */
const generateOptions = async () => {
  ...
  // Preenche com lista local para exibir sempre as siglas, sem depender da API.
  select.innerHTML = '<option value="">Selecione sua Criptomoeda</option>';

  [...currentCryptoCoins]
    .sort((a, b) => a.name.localeCompare(b.name))
    .forEach((coin) => {
      const opt = new Option(`${coin.name} (${coin.code})`, coin.code);
      select.appendChild(opt);
    });
};

// Aguardar DOM estar pronto antes de executar
